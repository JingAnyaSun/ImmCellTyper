...
#' @param type character string specifying the type of statistical analysis, with 'DA' stands for differential cell abundance analysis and 'DS'represents differential marker expression test facet by each cell subsets, default is 'DA'
#' @param group character string specifying the keyword for comparison, e.g.'condition'
#' @param flowSOM logical vector which is dependent on the clustering method used before, T means it was flowSOM, defalut is F
#' @param metaN character string specifying the code name of the number of clusters chosen by flowSOM
...
#' # Differential analysis of functional marker expression with in each cell cluster generated by Binary clustering or Phenograph, choose type as DE, n=2 will perform Mann-Whitney test and n>=3 will perform pairwise wilcoxon test
#' test<-statTest(sce,type='DE',group='condition')
#' # Differential analysis of functional marker expression within each cell cluster generated by flowSOM
#' test<-statTest(sce,type='DE',group='condition',flowSOM=T,metaN='meta10')
#'
statTest<-function(sce,abuntable=NULL,type='DA',group,flowSOM = F, metaN=NULL,posthoc=T, method='pairwise.wilcox.test'){
  ...
  # Differential cell abundance test across study conditions
  if(type=='DA'){
    message('Differential abundance test...')
    Sys.sleep(1)
    ...
    if(flowSOM==T){
        clusterid <- cluster_ids(sce,metaN)
      }else{
        clusterid <- sce$cluster_id
      }
    ...
    }else{ len<-length(unique(na.omit(df)[,group]))
    }
    ...
    state_exprs<- cbind(state_exprs, sample_id = sce$sample_id, condition = colData(sce)[,group],cluster_id=clusterid)
    ...
    message('Please input the right type of statistical analysis: DA, DE')
  }
}
